...
import { exactArrayBuffer, serializeError, type WorkerRequest, type WorkerResponse } from "./decompress-protocol.ts";
...
onmessage = async (event: MessageEvent<WorkerRequest>) => {
  ...
  try {
    ...
    postMessage({ id: message.id, error: serializeError(error) } satisfies WorkerResponse);
  }
};
