...
const _symmetricDecrypt: (data: Buffer, key: Buffer) => Buffer =
  _require("@doctormckay/steam-crypto").symmetricDecrypt;
...
export async function processChunkData(
  ...
  if (expectedSize !== undefined && (!Number.isSafeInteger(expectedSize) || expectedSize < 1 || expectedSize > MAX_CHUNK_BYTES)) {
    throw new Error(`Invalid expected size for chunk ${expectedSha1}`);
  }
  const decrypted = _symmetricDecrypt(encryptedData, key);
  const decompressed = await decompress(decrypted, expectedSize);
  if (expectedSize !== undefined && decompressed.length !== expectedSize) {
    throw new Error(`Chunk ${expectedSha1} has size ${decompressed.length}, expected ${expectedSize}`);
  }
...
function decompress(data: Buffer, expectedSize?: number): Buffer {
  ...
  if (data.length >= 23 && data.subarray(0, 4).toString("utf8") === "VSZa") {
    ...
    if (result.length !== declaredSize) throw new Error("Zstd: Decompressed size was not valid");
    if ((Bun.hash.crc32(result) >>> 0) !== expectedCrc) throw new Error("Zstd: CRC check failed on decompressed data");
    return result;
  }

  if (data.subarray(0, 3).toString("utf8") === "VZa") return decompressVzip(data, expectedSize);
  if (data.subarray(0, 4).toString("binary") === "PK\x03\x04") return decompressZip(data, expectedSize);
  throw new Error(`Unknown compression type: ${data.subarray(0, 4).toString("hex")}`);
}
...
function decompressZip(data: Buffer, expectedSize?: number): Buffer {
  ...
  if (typeof entry.header?.size === "number") validateDeclaredSize("ZIP", entry.header.size, expectedSize);
...
function decompressVzip(data: Buffer, manifestSize?: number): Buffer {
  if (data.length < 22 || data.subarray(data.length - 2).toString("utf8") !== "zv") {
  ...
  const compressed = Buffer.concat([properties, size, data.subarray(12, footerOffset)]);
  const result = lzma.decompressSync(compressed);
  if (result.length !== expectedSize) throw new Error("VZip: Decompressed size was not valid");
...
function validateDeclaredSize(format: string, declaredSize: number, expectedSize?: number): void {
  if (declaredSize < 1 || declaredSize > MAX_CHUNK_BYTES) {
    throw new Error(`${format}: Declared size exceeds the supported chunk limit`);
  }
...
export function downloadChunkData(
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("error", fail);
    req.setTimeout(REQUEST_TIMEOUT_MS, () => cancel(new Error(`HTTP request timed out after ${REQUEST_TIMEOUT_MS}ms`)));
    if (signal?.aborted) {
      onAbort();
      return;
    }
