...
async function downloadDepotContentLocked(
  ...
  const loadedPreviousManifest = previousManifestId
    ? await store.loadManifest(options.depotId, previousManifestId, inputs.depotKey)
    : undefined;
  ...
  await store.saveManifest(options.depotId, inputs.manifest.gid_manifest, inputs.manifestContents);
  ...
  await store.setInstalledManifestId(options.depotId, options.fileListPath ? null : inputs.manifest.gid_manifest);
  return result;
}
