...
export async function readDepotKey(path: string, depotId: number): Promise<Buffer> {
  ...
  for (const rawLine of contents.split(/[\r\n]+/u)) {
    ...
    if (!/^\d+$/u.test(id)) throw new Error(`Invalid depot ID in key line: ${rawLine}`);
    const parsedId = Number(id);
    if (!Number.isSafeInteger(parsedId) || parsedId < 0 || parsedId > 0xffffffff) {
      throw new Error(`Invalid depot ID in key line: ${rawLine}`);
    }
    if (!/^[0-9a-f]{64}$/iu.test(hex)) {
      throw new Error(`Depot ${parsedId} key must contain exactly 64 hexadecimal characters`);
    }
    if (keys.has(parsedId)) throw new Error(`Duplicate key for depot ${parsedId}`);
...
export function validateManifest(manifest: DepotManifest, depotId: number): void {
  if (manifest.depot_id !== depotId) {
    throw new Error(`Manifest belongs to depot ${manifest.depot_id}, expected ${depotId}`);
  }
  if (!manifest.gid_manifest || !Array.isArray(manifest.files)) {
    throw new Error("Manifest is missing required metadata or files");
  }
  if (!/^\d+$/u.test(manifest.gid_manifest)) throw new Error("Manifest has an invalid manifest ID");
  ...
  for (const file of manifest.files) {
    ...
    if (!/^[0-9a-f]{40}$/iu.test(file.sha_content)) {
      throw new Error(`Manifest contains an invalid file hash for ${file.filename}`);
    }
    let previousEnd = 0;
    for (const chunk of [...file.chunks].sort((left, right) => Number(left.offset) - Number(right.offset))) {
      if (!/^[0-9a-f]{40}$/iu.test(chunk.sha)) throw new Error(`Manifest contains an invalid chunk hash for ${file.filename}`);
      const offset = parseSafeInteger(chunk.offset, `chunk offset for ${file.filename}`);
      const originalSize = parseSafeInteger(chunk.cb_original, `chunk size for ${file.filename}`);
      const compressedSize = parseSafeInteger(chunk.cb_compressed, `compressed chunk size for ${file.filename}`);
      if (originalSize < 1 || originalSize > MAX_CHUNK_BYTES || compressedSize < 1 || compressedSize > MAX_CHUNK_BYTES) {
        throw new Error(`Manifest contains an unsupported chunk size for ${file.filename}`);
      }
      if (!Number.isInteger(chunk.crc) || chunk.crc < 0 || chunk.crc > 0xffffffff) {
        throw new Error(`Manifest contains an invalid chunk checksum for ${file.filename}`);
      }
    ...
    if (previousEnd !== size) throw new Error(`Manifest chunks do not exactly cover ${file.filename}`);
  }
}
