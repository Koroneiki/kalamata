...
export async function preflightManifestPaths(
  ...
  for (const file of files) {
    ...
    const collisionKey =
      process.platform === 'linux' ? outputPath : outputPath.toLowerCase()
    if (paths.has(collisionKey))
      throw new Error(`Duplicate manifest path: ${file.filename}`)
    paths.set(collisionKey, {
  ...
  const rootKey =
    process.platform === 'linux' ? resolve(root) : resolve(root).toLowerCase()
  for (const [childPath, child] of paths) {
    ...
    while (ancestor !== rootKey && ancestor !== dirname(ancestor)) {
      ...
      if (parent && !parent.directory) {
        throw new Error(
          `Manifest file path is an ancestor of ${child.filename}: ${parent.filename}`,
        )
      }
...
export async function stageTypeTransitions(
  ...
  const previous = new Map(
    previousFiles.map((file) => [manifestPathKey(file.filename), file]),
  )
  ...
  try {
    ...
    for (const file of [...currentFiles].sort(
      (left, right) => pathDepth(left.filename) - pathDepth(right.filename),
    )) {
      const outputPath = resolveOutputPath(root, file.filename)
      let info
      try {
      ...
      if (info.isSymbolicLink())
        throw new Error(`Manifest path is a symbolic link: ${file.filename}`)

      const wantsDirectory = Boolean(file.flags & DIRECTORY)
      if (
        (wantsDirectory && info.isDirectory()) ||
        (!wantsDirectory && info.isFile())
      )
        continue
      const oldFile = previous.get(manifestPathKey(file.filename))
      const oldWasDirectory = oldFile
        ? Boolean(oldFile.flags & DIRECTORY)
        : undefined
      if (oldWasDirectory === undefined || oldWasDirectory === wantsDirectory) {
        throw new Error(
          `Manifest path has an unexpected filesystem type: ${file.filename}`,
        )
      }
    ...
    try {
      ...
      throw new AggregateError(
        [error, restoreError],
        'Could not restore partially staged type transitions',
      )
    }
    throw error
  }
...
export async function restoreStagedPaths(paths: StagedPath[]): Promise<void> {
  ...
  if (errors.length > 0)
    throw new AggregateError(
      errors,
      'Could not restore paths after a failed type transition',
    )
}
...
export async function existingFileSize(
  path: string,
): Promise<number | undefined> {
  try {
    const info = await stat(path)
    if (!info.isFile())
      throw new Error(`Manifest file path is not a regular file: ${path}`)
    return info.size
  } catch (error) {
...
export async function rebuildWithChunkMatches(
  ...
  try {
      ...
      for (const match of matches) {
        ...
        while (copied < length) {
          const part = buffer.subarray(
            0,
            Math.min(buffer.length, length - copied),
          )
          const bytesRead = await readExactly(
            source,
            part,
            Number(match.source.offset) + copied,
          )
          if (bytesRead !== part.length)
            throw new Error(`Could not reuse chunk ${match.source.sha}`)
          await writeExactly(
            destination,
            part,
            Number(match.destination.offset) + copied,
          )
          copied += part.length
        }
      }
    ...
    try {
      ...
      throw new AggregateError(
        [error, restoreError],
        `Could not restore ${path}; original retained at ${stagingPath}`,
      )
    }
    throw error
  } finally {
