...
export class SteamContentClient implements ChunkClient {
  ...
  async downloadChunk(
    ...
    let token = server.usetokenauth === 1 || this.#tokenRequiredHosts.has(tokenCacheKey)
  ...
  async #getToken(appId: number, depotId: number, hostname: string, staleToken?: string): Promise<string> {
    ...
    if (token && staleToken !== undefined) {
      ...
      if (this.#cachedTokens.get(cacheKey) === token) this.#cachedTokens.delete(cacheKey);
      token = undefined;
    }
    if (!token) {
      const requestedToken = this.user.getCDNAuthToken(appId, depotId, hostname)
        .then((result) => result.token)
        .catch((error) => {
          if (this.#cachedTokens.get(cacheKey) === requestedToken) this.#cachedTokens.delete(cacheKey);
          throw error;
        });
