...
test("returns structured HTTP status errors", async () => {
  ...
  const error = await downloadChunkData(url, "cdn.example.test").catch((reason) => reason);
...
test("honors an already-aborted signal", async () => {
  ...
  await expect(downloadChunkData("http://127.0.0.1:1/chunk", "cdn.example.test", controller.signal))
...
test("rejects oversized response bodies", async () => {
  ...
  await expect(downloadChunkData(url, "cdn.example.test")).rejects.toThrow("Chunk response exceeds");
});
