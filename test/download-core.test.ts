...
describe("downloadManifest", () => {
  test("preallocates and downloads chunks at their offsets", async () => {
    ...
    const file = manifestFile("nested/file.bin", [chunk("first", 0, "abc"), chunk("second", 3, "xyz")]);

    const result = await downloadManifest(client, manifest(file), options(directory, false));

    expect(await readFile(join(directory, "nested/file.bin"), "utf8")).toBe("abcxyz");
    expect(result).toEqual({ manifestId: "123", downloadedBytes: 6, reusedBytes: 0 });
  });
  ...
  test("reuses valid chunks and replaces corrupt chunks when verifyAll is enabled", async () => {
    ...
    const file = manifestFile("file.bin", [chunk("first", 0, "abc"), chunk("second", 3, "xyz")]);

    const result = await downloadManifest(client, manifest(file), options(directory, true));
  ...
  test("moves valid old chunks and downloads only new chunks for a changed file", async () => {
    ...
    const oldFile = manifestFile("file.bin", [chunk("first", 0, "abc"), chunk("second", 3, "xyz")]);
    oldFile.sha_content = "old";
    const newFile = manifestFile("file.bin", [chunk("second", 0, "xyz"), chunk("first", 3, "abc"), chunk("third", 6, "NEW")]);
    ...
    expect(await readFile(join(directory, "file.bin"), "utf8")).toBe("xyzabcNEW");
    expect(result).toEqual({ manifestId: "123", downloadedBytes: 3, reusedBytes: 6 });
    expect(client.downloadChunk).toHaveBeenCalledTimes(1);
  });
  ...
  test("limits downloads and cleanup to files selected by the filter", async () => {
    ...
    const previous = manifest(manifestFile("excluded-old.bin", [chunk("old", 0, "old")]));

    await downloadManifest(fakeClient({ included: "yes" }), manifestFiles([included, excluded]), {
      ...options(directory, false),
    ...
    expect(await readFile(join(directory, "excluded-old.bin"), "utf8")).toBe("old");
  });
  ...
  test("cancels an active sibling download after a fatal worker failure", async () => {
    ...
    await expect(downloadManifest(client, manifestFiles([bad, hanging]), options(directory, false)))
  ...
  test("downloads one copy of a chunk referenced by multiple files", async () => {
    ...
    const result = await downloadManifest(client, manifestFiles([
  ...
  test("installs a known file-to-directory transition", async () => {
    ...
    await downloadManifest(fakeClient({ new: "new" }), manifestFiles([child, directoryEntry]), {
      ...options(directory, false),
      previousManifest: manifest(oldFile),
    });

    expect(await readFile(join(directory, "content/new.bin"), "utf8")).toBe("new");
  });
});
...
function manifestFiles(files: ManifestFile[]): DepotManifest {
  return {
    ...
    cb_disk_original: String(files.reduce((sum, file) => sum + Number(file.size), 0)),
    cb_disk_compressed: String(files.reduce((sum, file) => sum + Number(file.size), 0)),
    files,
  };
}

function manifestFile(filename: string, chunks: ManifestChunk[]): ManifestFile {
  const contents = Buffer.concat(chunks.map((item) => Buffer.from(item.sha === "first" ? "abc" : item.sha === "second" ? "xyz" : "")));
  return {
    filename,
...
function fakeClient(chunks: Record<string, string>) {
  ...
  const client: ChunkClient & { downloadChunk: ReturnType<typeof mock> } = {
    getContentServers: async () => ({ servers: [server] }),
    downloadChunk: mock(async (_appId: number, _depotId: number, sha: string) => {
