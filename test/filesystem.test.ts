...
import type { ManifestChunk } from '../src/backend/depot/manifests/types.ts'
...
test('rejects paths traversing an existing symlink', async () => {
  ...
  try {
    ...
    await expect(
      preflightManifestPaths(directory, [
        {
...
describe('chunk reuse', () => {
  test('validates and copies valid chunks while rebuilding', async () => {
    ...
    await rebuildWithChunkMatches(path, join(directory, 'staging.bin'), 6, [
      { source: chunks[0]!, destination: chunks[0]! },
    ])
  ...
  test('restores the original after rebuilding fails', async () => {
    ...
    await expect(
      rebuildWithChunkMatches(path, stagingPath, 8, [
