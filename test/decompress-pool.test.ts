...
import { DecompressPool } from "../src/decompress-pool.ts";
...
const symmetricEncrypt = require("@doctormckay/steam-crypto").symmetricEncrypt as
  (data: Buffer, key: Buffer, iv: Buffer) => Buffer;
...
describe("DecompressPool", () => {
  ...
  test("repeatedly decompresses Steam ZSTD chunks with Bun's native decoder", async () => {
    const pool = createPool(1);
    const expected = Array.from({ length: 4 }, (_, index) => Buffer.alloc(256 * 1024, index));

    for (const contents of expected) {
      expect(await pool.process(encryptedZstd(contents), sha1(contents))).toEqual(contents);
    }
  });

  test("repeatedly decompresses Steam VZip chunks with the native decoder", async () => {
    const pool = createPool(1);
    const expected = [Buffer.from("first legacy chunk"), Buffer.from("second legacy chunk")];

    for (const contents of expected) {
      expect(await pool.process(encryptedVzip(contents), sha1(contents))).toEqual(contents);
    }
  });
  ...
  test("rejects a VZip chunk with a malformed footer", async () => {
    ...
    await expect(pool.process(encryptedVzip(expected, (payload) => payload.write("xx", payload.length - 2)), sha1(expected)))
  ...
  test("rejects a VZip size that disagrees with the manifest", async () => {
    ...
    await expect(pool.process(encryptedVzip(expected), sha1(expected), expected.length + 1))
  ...
  test("rejects a VZip chunk with a CRC mismatch", async () => {
    ...
    await expect(pool.process(encryptedVzip(expected, (payload) => payload.writeUInt32LE(0, payload.length - 10)), sha1(expected)))
      .rejects.toThrow("CRC check failed");
  });

  test("processes queued chunks across multiple workers", async () => {
    const pool = createPool(2);
    const expected = Array.from({ length: 6 }, (_, index) => Buffer.alloc(64 * 1024, index));

    const actual = await Promise.all(
      expected.map((contents) => pool.process(encryptedZip(contents), sha1(contents))),
    );
  ...
  test("keeps a worker usable after a chunk validation error", async () => {
    ...
    await expect(pool.process(encryptedZip(expected), "0".repeat(40))).rejects.toThrow("SHA1 mismatch");
    expect(await pool.process(encryptedZip(expected), sha1(expected))).toEqual(expected);
  });

  test("rejects a container size that disagrees with the manifest", async () => {
    const pool = createPool(1);
    const expected = Buffer.from("valid contents");
    await expect(pool.process(encryptedZip(expected), sha1(expected), expected.length + 1))
      .rejects.toThrow("does not match the manifest");
  });
...
function encryptedVzip(contents: Buffer, mutate?: (payload: Buffer) => void): Buffer {
