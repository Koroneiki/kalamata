...
import { downloadDepotContent } from "../src/content-downloader.ts";
import { DepotConfigStore } from "../src/depot-config-store.ts";
import type { ChunkClient } from "../src/download-core.ts";
import type { DepotManifest, DownloadDepotOptions } from "../src/types.ts";
...
test("caches the local manifest and records completion after a successful run", async () => {
  ...
  expect(await readFile(join(directory, ".DepotDownloader/20_123.manifest"))).toEqual(contents);
...
test("leaves an incomplete marker when downloading fails", async () => {
  ...
  client.downloadChunk = mock(async () => { throw new Error("offline"); });

  await expect(downloadDepotContent(client, options(directory), {
    manifest,
  ...
  expect(await readFile(join(directory, ".DepotDownloader/20_123.manifest"))).toEqual(Buffer.from("manifest"));
});
...
test("ignores an invalid cached previous manifest", async () => {
  ...
  expect(JSON.parse(await readFile(join(directory, ".DepotDownloader/depot.config.json"), "utf8")))
    .toEqual({ version: 1, installedManifestIds: { "20": "123" } });
});

test("does not record a filtered download as a complete manifest", async () => {
  directory = await mkdtemp(join(tmpdir(), "depot-content-"));
  const filteredOptions = { ...options(directory), fileListPath: "selection.txt" };
  ...
  const config = JSON.parse(await readFile(join(directory, ".DepotDownloader/depot.config.json"), "utf8"));
...
function fakeClient(): ChunkClient {
  return {
    getContentServers: async () => ({ servers: [{ Host: "cdn.example.test" }] }),
