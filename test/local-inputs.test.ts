...
test("parses and validates the checked-in Balatro manifest without network access", async () => {
  const key = Buffer.from("16261e41d3e864018778d4a1d81658521a67d9ffb8543ea7e3e21f0685721af1", "hex");
  const contents = await readFile(join(import.meta.dir, "fixtures", "2379781_3512319404653808464.manifest"));
  ...
  expect(manifest.files.reduce((sum, file) => sum + file.chunks.length, 0)).toBe(75);
...
function basicManifest(): DepotManifest {
  return {
    ...
    files: [{
      ...
      chunks: [{ sha: "02".repeat(20), crc: 1, offset: "0", cb_original: 3, cb_compressed: 3 }],
    }],
  };
}
