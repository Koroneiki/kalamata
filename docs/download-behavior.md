...
| Resume              | Trusts a valid completion ledger and correctly sized staged files, then downloads missing chunks without reclassifying live installed files. A resumed repair rechecks retained files because repair guarantees installed integrity. The verifying phase applies executable mode; it does not rehash ledger-marked staged bytes.                            |
...
By default, loading a library application's details acquires the latest missing or invalid base-game and DLC depot keys and manifests for the selected platforms. Unknown, unused, redistributable, package-excluded, and platform-filtered depots are excluded. The persisted setting can disable this and restore per-depot controls. Manual acquisition requests a missing key first. It then requests the manifest even if no key was acquired. Key acquisition checks SQLite, the base application's GitHub Lua source, and the shared GitHub JSON source in that order. The Lua source can include DLC depot keys. At startup, HTTP validators refresh the JSON fallback only when it has changed. A failed refresh leaves the existing valid copy in place.

Hubcap is the last depot-key source and the fallback manifest source. Its API key is stored in the settings row in SQLite. Before checking quota for keys, the backend reads Hubcap's free depot-ID list and continues only for unresolved IDs that Hubcap reports as available. Before checking quota for a manifest, it reads the free app manifest contents and requires an exact depot ID, manifest ID, and canonical filename match. If the requested resource is not listed, it skips both usage and paid requests. Before each new Hubcap Lua or manifest ZIP download, the backend checks the free usage endpoint. Missing or invalid credentials, unavailable preflight data, and exhausted quota skip the download. When ten or fewer daily requests remain, each download needs confirmation. A confirmation permits one request only. Successful app Lua and manifest ZIP responses are cached in memory and shared by concurrent acquisitions, so reusing one does not spend quota or prompt again. Automatic, manual, update-preparation, and custom-manifest acquisition use this same path.

Request-code lookups are sent one at a time to `manifest.manifestdex.com`. Steam CDN downloads do not use that lock. Concurrent requests for the same depot and manifest share one acquisition. Quitting rejects new acquisition work and cancels unfinished acquisitions before Steam and database disposal.

Kalamata sends the displayed public manifest ID to `manifest.manifestdex.com` for a request code, then acquires the manifest from Steam CDN through its anonymous session. If the request-code lookup fails or returns an invalid response, Hubcap is checked as a fallback. An advertised Hubcap manifest is downloaded from the app ZIP only after the same quota checks and low-quota confirmation used for depot keys. Before saving a manifest from either source, the backend validates the container marker and embedded depot and manifest IDs. Readable filenames, metadata, and chunk layouts receive full structural validation. A valid managed manifest is not downloaded again, but a missing or invalid one can be replaced. Startup removes registrations whose managed files are missing. It does not import files copied into the manifest directory outside backend ingestion.
...
A preview does not occupy the operation queue, reserve an install path, inspect installed files, or save the depot selection. It removes completely overridden depots from the in-memory selection draft. Partial overlaps remain selected and show which later-mounted depots take priority. Logical size change is target size minus source size. Maximum temporary disk space totals changed effective file sizes. The network upper bound totals unique compressed chunks before local reuse. Estimates assume non-configuration files recorded by installed source manifests are intact. Temporary-space estimates omit target files that can remain in place. Download estimates subtract reusable chunks from non-configuration source files needed to stage the remaining targets. Execution checks the live filesystem and may require up to the displayed upper bounds when files cannot be reused. Arbitrary partial files outside the installed source projection are not chunk-reuse candidates.
