...
Before first installation, depot selection edits are saved immediately. Starting that installation reserves its output path even though no depot is installed yet; UI installation state and Verify availability therefore derive from installed-depot records rather than the reservation. For an installed application, edits remain an in-memory draft until Update or Uninstall is confirmed; reconciliation saves that selection after planning and before staging, so it can remain selected when later staging fails. Stored selections are not pruned when Steam metadata temporarily omits a depot.
