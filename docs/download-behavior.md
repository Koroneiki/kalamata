...
Hubcap is the last depot-key source and runs only when the free sources leave requested keys missing. Its API key is stored in the settings row in SQLite. Before checking quota, the backend reads Hubcap's free depot-ID list and continues only for unresolved IDs that Hubcap reports as available. If none are listed, it skips both usage and Lua requests. Before each new Hubcap Lua download, the backend checks the free usage endpoint. Missing or invalid credentials, unavailable preflight data, and exhausted quota skip the download. When ten or fewer daily requests remain, each download needs confirmation. A confirmation permits one request only. Successful app Lua responses are cached in memory and shared by concurrent acquisitions, so reusing one does not spend quota or prompt again. Automatic, manual, update-preparation, and custom-manifest acquisition use this same path.

Request-code lookups are sent one at a time to `manifest.opensteamtool.com`. Steam CDN downloads do not use that lock. Concurrent requests for the same depot and manifest share one acquisition. Quitting rejects new acquisition work and cancels unfinished acquisitions before Steam and database disposal.
